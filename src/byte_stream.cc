...
ByteStream::ByteStream( uint64_t capacity ) 
  : impl_(make_shared<ByteStreamImpl>(capacity)) {}

// 拷贝构造
ByteStream::ByteStream( const ByteStream& other)
  : impl_(make_shared<ByteStreamImpl>(*other.impl_)) {}
...
void Writer::push( string data )
{
  ...
  uint64_t len_to_write = min(available, static_cast<uint64_t>(data.size()));
...
string_view Reader::peek() const
{
  ...
  std::string_view sv(current_block);
...
void Reader::pop( uint64_t len )
{
  ...
  while ( len > 0 ) {
    ...
    uint64_t current_str_size = front_str.size();
    uint64_t remain_in_front = current_str_size - impl_->front_offset_;
